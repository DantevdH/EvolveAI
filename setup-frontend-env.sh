...
echo "🔧 Generating frontend .env for local development..."
...
if [ -z "$SUPABASE_JSON" ]; then
  echo "❌ Error: Could not get Supabase status. Is Supabase running?"
...
if ! command -v python3 >/dev/null 2>&1; then
  echo "❌ Error: python3 not found. Install python3 to continue."
  exit 1
fi
