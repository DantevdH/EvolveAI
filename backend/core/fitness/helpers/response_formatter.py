...
class ResponseFormatter:
    ...
    @staticmethod
    def format_responses(responses: Dict[str, Any], questions: Optional[List[AIQuestion]] = None) -> str:
        """
        ...
        for question_id, response in responses.items():
            try:
                ...
                response_str = ResponseFormatter._format_single_response(response, question)
    ...
    @staticmethod
    def _format_single_response(response: Any, question: Optional[AIQuestion] = None) -> str:
        """
        ...
        if isinstance(response, list):
            ...
            if question and question.options:
                return ResponseFormatter._format_multiple_choice_response(response, question)
            else:
    ...
    @staticmethod
    def _format_multiple_choice_response(response: List[str], question: Optional[AIQuestion] = None) -> str:
        ...
        if len(response) == 0:
            ...
            if question and question.options:
                option = next((opt for opt in question.options if opt.value == str(response[0])), None)
    ...
    @staticmethod
    def _format_numeric_response(response: float, question: Optional[AIQuestion] = None) -> str:
