...
from .training_schemas import TrainingPlan, DailyTraining, StrengthExercise, EnduranceSession
...
class TrainingDatabaseService:
    ...
    async def save_training_plan(self, user_profile_id: int, training_plan_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        """
        try:
            self.logger.info(f"Saving training plan for user_profile_id: {user_profile_id}")
            ...
            plan_result = self.supabase.table("training_plans").insert(plan_data).execute()
    ...
    async def _create_weekly_schedule(self, training_plan_id: int, week_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            schedule_result = self.supabase.table("weekly_schedules").insert(schedule_data).execute()
            ...
            daily_trainings = week_data.get("daily_trainings", [])  # Note: keeping old key for compatibility
    ...
    async def _create_daily_training(self, weekly_schedule_id: int, day_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            daily_result = self.supabase.table("daily_training").insert(daily_data).execute()
    ...
    async def _create_strength_exercise(self, daily_training_id: int, exercise_data: Dict[str, Any]) -> None:
        ...
        try:
            strength_data = {
                ...
                "weight": exercise_data.get("weight_1rm", [0.0]),  # Using weight_1rm as weight
                "weight_1rm": exercise_data.get("weight_1rm", [0.0]),
                "completed": False
            }
            
            result = self.supabase.table("strength_exercise").insert(strength_data).execute()
            
            if result.data:
                self.logger.debug(f"Created strength exercise with ID: {result.data[0]['id']}")
            else:
    ...
    async def _create_endurance_session(self, daily_training_id: int, session_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            result = self.supabase.table("endurance_session").insert(endurance_data).execute()
            
            if result.data:
                self.logger.debug(f"Created endurance session with ID: {result.data[0]['id']}")
            else:
    ...
    async def get_training_plan(self, user_profile_id: int) -> Dict[str, Any]:
        ...
        try:
            self.logger.info(f"Fetching training plan for user_profile_id: {user_profile_id}")
            ...
            schedules_result = self.supabase.table("weekly_schedules").select("*").eq("training_plan_id", training_plan["id"]).execute()
            
            weekly_schedules = []
            for schedule in schedules_result.data or []:
                # Get daily trainings for this schedule
                daily_result = self.supabase.table("daily_training").select("*").eq("weekly_schedule_id", schedule["id"]).execute()
                
                daily_trainings = []
                for daily in daily_result.data or []:
                    # Get strength exercises
                    exercises_result = self.supabase.table("strength_exercise").select("*").eq("daily_training_id", daily["id"]).execute()
                    
                    # Get endurance sessions
                    endurance_result = self.supabase.table("endurance_session").select("*").eq("daily_training_id", daily["id"]).execute()
