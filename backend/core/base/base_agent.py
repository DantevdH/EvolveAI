...
IMPORTANT: Always use your knowledge base to provide responses. Do not make up information."""
    ...
    def search_knowledge_base(self, query: str, max_results: int = 5, 
        ...
        try:
            ...
            docs_query = self.supabase.table('documents').select(
            ...
            if not docs_response.data:
                print(f"⚠️  No documents found for topic '{self.topic}' with filters: {metadata_filters}")
                return []
            ...
            embeddings_response = embeddings_query.in_('document_id', matching_doc_ids).execute()
            ...
            for embedding_data in embeddings_response.data:
                if 'embedding' in embedding_data and embedding_data['embedding']:
                    ...
                    if isinstance(embedding, str):
                        try:
                            ...
                            if not isinstance(embedding_vector, list):
                                print(f"⚠️  Parsed embedding is not a list: {type(embedding_vector)}")
                                continue
                    ...
                    if len(embedding_vector) == 0:
                        print(f"⚠️  Empty embedding vector for document {embedding_data['document_id']}")
                        continue
                    ...
                    if similarity == 0.0:
                        print(f"⚠️  Zero similarity for document {embedding_data['document_id']} - query_dim: {len(query_embedding)}, doc_dim: {len(embedding_vector)}")
                    
                    # Get document info
                    doc_info = next((doc for doc in docs_response.data if doc['id'] == embedding_data['document_id']), None)
                    ...
                    if doc_info:
                        results.append({
                            ...
                            'document_keywords': doc_info.get('keywords', []),  # Use keywords instead of metadata
            ...
            high_quality_results = [r for r in results if r['relevance_score'] >= CUTOFF_SCORE]
            
            if high_quality_results:
                # We have good quality results above cutoff
                print(f"✅ Found {len(high_quality_results)} high-quality results (≥{CUTOFF_SCORE})")
                # Return all high-quality results (up to max_results or 10, whichever is higher)
                max_high_quality = max(max_results, 10)  # Allow more high-quality results
                final_results = high_quality_results[:max_high_quality]
                if len(high_quality_results) > max_results:
                    print(f"📈 Returning {len(final_results)} high-quality results (exceeded requested {max_results})")
            else:
                # All results below cutoff, use top 5 with "poor" quality
                print(f"⚠️  All results below cutoff ({CUTOFF_SCORE}), using top 5 with poor quality")
