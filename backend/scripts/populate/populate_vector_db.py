...
class PDFVectorDBPopulator:
    ...
    def generate_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            for attempt in range(max_retries):
                try:
                    logger.info(
                        f"Generating embeddings for batch {i//batch_size + 1}/{(len(texts) + batch_size - 1)//batch_size} ({len(batch)} chunks)"
                    )
    ...
    def _detect_difficulty(self, text: str) -> str:
        ...
        if any(
            word in text_lower
            for word in ["beginner", "basic", "starting", "first time"]
        ):
            return "beginner"
        elif any(
            word in text_lower
            for word in ["advanced", "expert", "professional", "elite"]
        ):
            return "advanced"
        else:
            return "intermediate"
