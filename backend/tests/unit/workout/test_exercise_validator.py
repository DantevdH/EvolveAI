...
class TestExerciseValidator:
    ...
    def mock_validator(self, mock_exercise_selector):
        ...
        with patch(
            "core.training.helpers.exercise_validator.ExerciseSelector",
    ...
    def test_validate_training_plan_success(self, mock_validator, sample_training_plan):
        ...
        validated_plan, messages = mock_validator.validate_training_plan(
    ...
    def test_validate_training_plan_all_valid_ids(self, mock_validator):
        ...
        mock_validator.exercise_selector.validate_exercise_ids.return_value = (
            ["1", "2"],
            [],
        )
    ...
    def test_validate_training_plan_error_handling(
        self, mock_validator, sample_training_plan
    ):
        """Test training plan validation error handling."""
        # Make the mock raise an exception
        mock_validator.exercise_selector.validate_exercise_ids.side_effect = Exception(
            "Database error"
        )
    ...
    def test_find_best_match_by_similarity_success(self, mock_validator):
        ...
        mock_validator.vectorizer.fit_transform = Mock(
            return_value=np.array([[1, 0, 1], [0, 1, 0], [1, 1, 1]])
        )

        with patch(
            "sklearn.metrics.pairwise.cosine_similarity",
            return_value=np.array([[0.8, 0.3]]),
        ):
            best_match = mock_validator._find_best_match_by_similarity(
                target_description, candidates
            )
    ...
    def test_find_best_match_by_similarity_low_similarity(self, mock_validator):
        ...
        candidates = [
            {
                "name": "Exercise 1",
                "description": "Desc 1",
                "main_muscle": "Muscle 1",
                "target_area": "Target 1",
            },
            {
                "name": "Exercise 2",
                "description": "Desc 2",
                "main_muscle": "Muscle 2",
                "target_area": "Target 2",
            },
        ]
        ...
        with patch(
            "sklearn.metrics.pairwise.cosine_similarity",
            return_value=np.array([[0.05, 0.02]]),
        ):
            best_match = mock_validator._find_best_match_by_similarity(
                target_description, candidates
            )
                    ...
                    "daily_trainings": [
                        {
                            ...
                            "warming_up_instructions": "",
                            "daily_justification": "",
                            "cooling_down_instructions": "",
                        },
                        {
                            "day_of_week": "Tuesday",
                            "is_rest_day": False,
                    ...
                    "daily_trainings": [
                        {
                            ...
                            "warming_up_instructions": "Warmup",
                            "daily_justification": "Just",
                            "cooling_down_instructions": "Cool",
                        },
                        {
                            "day_of_week": "Tuesday",
                            "is_rest_day": False,
    ...
    def test_validate_training_structure_no_exercises_on_training_day(
        self, mock_validator
    ):
                    ...
                    "daily_trainings": [
                        {
                            ...
                            "daily_justification": "",
                            "cooling_down_instructions": "",
                        },  # Training day with no exercises
                        {
                            "day_of_week": "Tuesday",
                            "is_rest_day": True,
                            "exercises": [],
                    ...
                    "daily_trainings": [
                        {
                            "day_of_week": "Monday",
                            "is_rest_day": False,
                            "exercises": [{"name": "Exercise without ID"}],
                            "warming_up_instructions": "",
                            "daily_justification": "",
                            "cooling_down_instructions": "",
                        },
...
class TestExerciseValidatorEdgeCases:
    ...
    def test_validate_training_plan_malformed_structure(
        self, mock_validator_edge_cases
    ):
        ...
        validated_plan, messages = mock_validator_edge_cases.validate_training_plan(
            malformed_plan
        )
    ...
    def test_cosine_similarity_calculation_edge_cases(self, mock_validator_edge_cases):
        ...
        mock_validator_edge_cases.vectorizer.fit_transform = Mock(
            return_value=np.array([[1, 0], [0, 1], [1, 1]])
        )

        with patch(
            "sklearn.metrics.pairwise.cosine_similarity",
            return_value=np.array([[0.5, 0.3]]),
        ):
            best_match = mock_validator_edge_cases._find_best_match_by_similarity(
                short_desc, candidates
            )
            assert best_match is not None
    ...
    def test_muscle_extraction_edge_cases(self, mock_validator_edge_cases):
        ...
        long_name = (
            "Super Advanced Compound Multi-Joint Barbell Squat Variation with Twist"
        )
    ...
    def test_extract_and_validate_exercises_structure_validation(self, mock_validator):
        ...
        invalid_plan = {"title": "Test Plan", "weekly_schedules": []}  # No weeks
    ...
    def test_fix_invalid_exercises_no_invalid_ids(self, mock_validator):
            ...
            "weekly_schedules": [
                {
                    ...
                    "daily_trainings": [
                        {
                            "day_of_week": "Monday",
                            "exercises": [{"exercise_id": 1, "name": "Squat"}],
                        }
                    ],
                }
            ],
        ...
        exercise_data = {"valid_ids": [1], "invalid_ids": [], "exercise_locations": []}
    ...
    def test_fix_invalid_exercises_with_replacement_cache(self, mock_validator):
        ...
        mock_validator._find_replacement_exercise = Mock(
            return_value={
                "id": 1,
                "name": "Valid Exercise",
        ...
        assert (
            result["weekly_schedules"][0]["daily_trainings"][0]["exercises"][0]["id"]
            == 1
        )
    ...
    def test_fix_invalid_exercises_remove_exercise(self, mock_validator):
        ...
        assert (
            len(result["weekly_schedules"][0]["daily_trainings"][0]["exercises"]) == 0
        )
    ...
    def test_find_replacement_exercise_no_target_muscle(self, mock_validator):
        ...
        original_exercise = {"name": "Unknown Exercise", "difficulty": "Beginner"}
    ...
    def test_find_replacement_with_similarity_cache_hit(self, mock_validator):
        ...
        # Pre-populate cache
        mock_validator._similarity_cache[f"sim_{cache_key}_{len(candidates)}"] = {
            "id": 1,
            "name": "Cached Exercise",
        }

        result = mock_validator._find_replacement_with_similarity(
            target_description, candidates, cache_key
        )
    ...
    def test_find_replacement_with_similarity_cache_miss(self, mock_validator):
        ...
        mock_validator._find_best_match_by_similarity = Mock(
            return_value={"id": 1, "name": "Bench Press"}
        )
    ...
    def test_find_best_match_by_similarity_success(self, mock_validator):
        ...
        with patch(
            "core.training.helpers.exercise_validator.cosine_similarity"
        ) as mock_cosine:
            mock_cosine.return_value = [
                [0.9, 0.1]
            ]  # High similarity for first, low for second

            result = mock_validator._find_best_match_by_similarity(
                target_description, candidates
            )
    ...
    def test_validate_training_structure_no_weeks(self, mock_validator):
        ...
        training_plan = {"title": "Test Plan", "weekly_schedules": []}
            ...
            "weekly_schedules": [
                {
                    "week_number": 1,
                    "daily_trainings": [
                        {"day_of_week": f"Day {i}"} for i in range(20)
                    ],  # 20 days
                }
            ],
        }
    ...
    def test_validate_training_structure_no_days(self, mock_validator):
        ...
        training_plan = {
            "title": "Test Plan",
            "weekly_schedules": [{"week_number": 1, "daily_trainings": []}],
        }
