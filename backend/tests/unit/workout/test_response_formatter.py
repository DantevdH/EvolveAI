...
class TestResponseFormatter:
    ...
    def test_format_responses_with_questions(self, sample_responses, sample_questions):
        ...
        assert (
            "Q: How motivated are you to achieve your goals?\nA: 4/5 rating (1=Not motivated, 5=Very motivated)"
            in result
        )
        assert (
            "Q: Rate your current training level\nA: 7/10 rating (1=Beginner, 10=Expert)"
            in result
        )
    ...
    def test_format_responses_error_handling(self, sample_questions):
        ...
        result = ResponseFormatter.format_responses(
            malformed_responses, sample_questions
        )
        assert isinstance(result, str)
        assert len(result) > 0
    ...
    def test_real_world_example(self, sample_responses, sample_questions):
        ...
        result_with_context = ResponseFormatter.format_responses(
            sample_responses, sample_questions
        )
        ...
        assert len(result_with_context) > len(
            result_without_context
        )  # With context should be more detailed
