...
class TestRAGTool:
    ...
    @pytest.fixture
    def rag_service(self, mock_base_agent):
    ...
    def test_rag_service_initialization(self, mock_base_agent):
        """Test RAGTool initialization."""
        rag_service = RAGTool(mock_base_agent)

        assert rag_service.base_agent == mock_base_agent

    def test_extract_metadata_filters_difficulty_beginner(self, rag_service):
        """Test metadata extraction for beginner difficulty."""
        query = "I'm a beginner and want to start working out"
        filters = rag_service.extract_metadata_filters(query)

        assert filters["difficulty_level"] == "beginner"
        assert "body_part" not in filters

    def test_extract_metadata_filters_difficulty_intermediate(self, rag_service):
    ...
    def test_extract_metadata_filters_difficulty_advanced(self, rag_service):
    ...
    def test_extract_metadata_filters_body_parts(self, rag_service):
    ...
    def test_extract_metadata_filters_sport_types(self, rag_service):
    ...
    def test_extract_metadata_filters_equipment(self, rag_service):
    ...
    def test_extract_metadata_filters_training_frequency(self, rag_service):
    ...
    def test_extract_metadata_filters_goals(self, rag_service):
    ...
    def test_extract_metadata_filters_multiple_filters(self, rag_service):
        """Test metadata extraction with multiple filters."""
        query = (
            "I'm a beginner and want to work on my chest with dumbbells for muscle gain"
        )
        filters = rag_service.extract_metadata_filters(query)
    ...
    def test_extract_metadata_filters_no_matches(self, rag_service):
    ...
    def test_extract_metadata_filters_case_insensitive(self, rag_service):
    ...
    def test_perform_hybrid_search_success(self, rag_service, sample_search_results):
        """Test successful hybrid search."""
        # Mock the base agent to return results
        rag_service.base_agent.search_knowledge_base.return_value = sample_search_results

        query = "beginner chest training"
        results = rag_service.perform_hybrid_search(query, max_results=5)

        # Should call search_knowledge_base with metadata filters
        rag_service.base_agent.search_knowledge_base.assert_called_once()
        call_args = rag_service.base_agent.search_knowledge_base.call_args
    ...
    def test_perform_hybrid_search_few_results_fallback(self, rag_service):
        """Test hybrid search with fallback to broader search."""
        # Mock filtered search to return few results
        rag_service.base_agent.search_knowledge_base.side_effect = [
        ...
        assert rag_service.base_agent.search_knowledge_base.call_count == 2

        # Should return combined results
        assert len(results) > 0

    def test_perform_hybrid_search_no_results(self, rag_service):
        """Test hybrid search with no results."""
        # Mock search to return no results
        rag_service.base_agent.search_knowledge_base.return_value = []
    ...
    def test_re_rank_results(self, rag_service, sample_search_results):
        """Test result re-ranking."""
        query = "beginner chest training"
        ranked_results = rag_service._re_rank_results(query, sample_search_results)
    ...
    def test_re_rank_results_empty_list(self, rag_service):
        """Test re-ranking with empty results."""
        results = rag_service._re_rank_results("query", [])
        assert results == []

    def test_re_rank_results_no_relevance_score(self, rag_service):
        ...
        ranked_results = rag_service._re_rank_results("query", results_without_scores)

        # Should handle missing scores gracefully
        assert all("final_score" in result for result in ranked_results)

    def test_augment_context_success(self, rag_service, sample_search_results):
        """Test successful context augmentation."""
        # Mock hybrid search to return results
        rag_service.perform_hybrid_search = Mock(return_value=sample_search_results)

        query = "beginner training"
        augmented = rag_service.augment_context(query, max_context_length=1000)
    ...
    def test_augment_context_no_results(self, rag_service):
        """Test context augmentation with no search results."""
        # Mock hybrid search to return no results
        rag_service.perform_hybrid_search = Mock(return_value=[])

        query = "very specific query"
        augmented = rag_service.augment_context(query)

        # Should return original query unchanged
        assert augmented == query

    def test_augment_context_max_length_limit(self, rag_service):
        ...
        rag_service.perform_hybrid_search = Mock(return_value=long_results)

        query = "test query"
        augmented = rag_service.augment_context(query, max_context_length=500)

        # Should respect max length limit
        assert len(augmented) <= len(query) + 500

    def test_get_search_insights(self, rag_service, sample_search_results):
        """Test getting search insights."""
        # Mock the required methods
        rag_service.extract_metadata_filters = Mock(
            return_value={"difficulty_level": "beginner"}
        )
        rag_service.perform_hybrid_search = Mock(return_value=sample_search_results)

        query = "beginner chest training"
        insights = rag_service.get_search_insights(query)
    ...
    def test_get_search_insights_no_filters(self, rag_service, sample_search_results):
        """Test search insights with no metadata filters."""
        # Mock no metadata filters
        rag_service.extract_metadata_filters = Mock(return_value={})
        rag_service.perform_hybrid_search = Mock(return_value=sample_search_results)
    ...
    def test_get_search_insights_empty_results(self, rag_service):
    ...
    def test_metadata_filter_priority(self, rag_service):
    ...
    def test_edge_case_empty_query(self, rag_service):
        """Test edge case with empty query."""
        filters = rag_service.extract_metadata_filters("")
        assert filters == {}
    ...
    def test_edge_case_special_characters(self, rag_service):
