...
def test_exercise_selector_intermediate_difficulty(exercise_selector):
    ...
    assert (
        len(result) > 0
    ), "Should return formatted exercise string for intermediate level"
...
def test_exercise_validator_id_validation(exercise_validator):
                ...
                "daily_trainings": [
                    {
                        ...
                        "is_rest_day": False,
                        "exercises": [
                            {"exercise_id": id_val, "sets": 3, "reps": [8, 8, 8]}
    ...
    print(
        f"✅ Validated training plan with {len(test_ids)} exercises: {len(messages)} messages"
    )
...
def test_exercise_validator_invalid_exercise_handling(
    exercise_validator, mock_training_plan
):
...
def test_training_coach_initial_questions_generation(
    training_coach, sample_user_profiles
):
    ...
    user_profile_dict = (
        muscle_user.model_dump()
        if hasattr(muscle_user, "model_dump")
        else muscle_user.__dict__
    )
...
def test_training_coach_follow_up_questions_generation(
    training_coach, sample_user_profiles
):
    ...
    result = training_coach.generate_follow_up_questions(
        user_profile_dict, initial_responses
    )
...
def test_training_coach_training_plan_generation(training_coach, sample_user_profiles):
    ...
    if result["success"]:
        ...
        print(
            f"⚠️ Training plan generation failed: {result.get('error', 'Unknown error')}"
        )
...
def test_training_coach_profile_processing(training_coach, sample_user_profiles):
    ...
    print(
        f"✅ Profile processing: {user_profile_dict['primary_goal']} - {user_profile_dict['experience_level']}"
    )
...
def test_training_generation_with_exercise_candidates(
    training_coach, sample_user_profiles
):
    ...
    # Convert UserProfileSchema to dict for the API
    user_profile_dict = (
        weight_loss_user.model_dump()
        if hasattr(weight_loss_user, "model_dump")
        else weight_loss_user.__dict__
    )
    ...
    if exercise_result and exercise_result != "No exercises available":
        ...
        plan_result = training_coach.generate_training_plan(
...
def test_complete_training_generation_workflow(training_coach, sample_user_profiles):
    ...
    if exercise_result and exercise_result != "No exercises available":
        ...
        print(
            "   ⚠️ No exercise candidates found (database might be empty for this profile)"
        )
    ...
    if plan_result["success"]:
        ...
        print(
            f"   ✅ Generated training plan: {training_plan.get('plan_name', 'Unknown')}"
        )
    else:
        print(
            f"   ⚠️ Plan generation failed: {plan_result.get('error', 'Unknown error')}"
        )
...
def test_error_handling_integration(training_coach, sample_user_profiles):
    ...
    try:
        ...
        print(
            f"   ✅ Invalid profile data handled gracefully: {len(result.questions)} questions"
        )
...
def test_large_exercise_list_handling(exercise_validator):
    ...
    large_training_plan = {"title": "Large Test Plan", "weekly_schedules": []}
    ...
    print(
        f"📊 Testing large training plan: {len(large_training_plan['weekly_schedules'])} weeks"
    )
