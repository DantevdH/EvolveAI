...
class TestTrainingFlowIntegration:
    ...
    def sample_initial_questions(self):
        ...
        return [
            AIQuestion(
                ...
                options=[
                    QuestionOption(
                        id="opt1", text="Build Muscle", value="muscle_building"
                    ),
                    QuestionOption(id="opt2", text="Lose Weight", value="weight_loss"),
                    QuestionOption(
                        id="opt3", text="Increase Strength", value="strength"
                    ),
                ],
            ),
            AIQuestion(
    ...
    def sample_initial_responses(self):
        ...
        return {"q1": "muscle_building", "q2": 4}
        ...
        return [
            AIQuestion(
                ...
                options=[
                    ...
                    QuestionOption(
                        id="eq3", text="Bodyweight Only", value="bodyweight"
                    ),
                ],
            )
        ]
    ...
    def mock_database_service(self):
        ...
        service.create_user_profile = AsyncMock(
            return_value={"success": True, "data": {"id": 1, "user_id": "test-user-id"}}
        )
        service.update_user_profile = AsyncMock(return_value={"success": True})
        service.get_user_profile_by_user_id = AsyncMock(
            return_value={"success": True, "data": {"id": 1, "user_id": "test-user-id"}}
        )
        service.save_training_plan = AsyncMock(
            return_value={"success": True, "data": {"training_plan_id": 1}}
        )
        return service
    ...
    @pytest.mark.asyncio
    async def test_complete_training_flow_success(
        self, sample_personal_info, mock_database_service, mock_training_coach
    ):
        """Test the complete training flow from initial questions to plan generation."""

        # Mock JWT extraction
        with patch(
            "core.training.training_api.extract_user_id_from_jwt",
            return_value="test-user-id",
        ):
            with patch("core.training.training_api.db_service", mock_database_service):
                with patch(
                    "core.training.training_api.get_training_coach",
                    return_value=mock_training_coach,
                ):

                    # Test 1: Initial Questions
                    initial_request = InitialQuestionsRequest(
                        personal_info=sample_personal_info, jwt_token="test-jwt-token"
                    )

                    response = await router.routes[0].endpoint(
                        initial_request, mock_training_coach
                    )
                    ...
                    follow_up_response = await router.routes[1].endpoint(
                        follow_up_request, mock_training_coach
                    )
                    ...
                    outline_response = await router.routes[2].endpoint(
                        outline_request, mock_training_coach
                    )
                    ...
                    outline_dict = (
                        outline_response["data"]["outline"].model_dump()
                        if hasattr(outline_response["data"]["outline"], "model_dump")
                        else outline_response["data"]["outline"]
                    )
                    ...
                    plan_response = await router.routes[3].endpoint(
                        plan_request, mock_training_coach
                    )
    ...
    @pytest.mark.asyncio
    async def test_initial_questions_database_failure(
        self, sample_personal_info, mock_database_service, mock_training_coach
    ):
    ...
    @pytest.mark.asyncio
    async def test_training_plan_generation_with_exercise_decision(
        self, sample_personal_info, mock_training_coach
    ):
        ...
        with patch("core.training.training_coach.openai") as mock_openai:
            mock_openai.chat.completions.parse.return_value.choices[
                0
            ].message.parsed = mock_decision

            # Mock exercise selector
            with patch(
                "core.training.training_coach.ExerciseSelector"
            ) as mock_exercise_selector:
                ...
                mock_selector_instance.get_formatted_exercises_for_ai.return_value = (
                    "Exercise data formatted for AI"
                )
    ...
    def test_database_service_training_type_handling(self):
        ...
        for daily_training in daily_trainings:
            training_type = daily_training.get(
                "training_type",
                "recovery" if daily_training.get("is_rest_day", False) else "strength",
            )
    ...
    def test_pydantic_serialization_consistency(self):
        ...
        question = AIQuestion(
            id="test", text="Test question", response_type=QuestionType.MULTIPLE_CHOICE
        )
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_initial_questions(
        self, sample_personal_info, mock_training_coach
    ):
    ...
    def test_mock_data_consistency(self):
        ...
        for weekly_schedule in training_plan.weekly_schedules:
            for daily_training in weekly_schedule.daily_trainings:
                ...
                assert daily_training.training_type in [
                    "strength",
                    "endurance",
                    "mixed",
                    "recovery",
    ...
    async def test_jwt_token_validation(
        ...
        with patch(
            "core.training.training_api.extract_user_id_from_jwt",
            side_effect=ValueError("Invalid JWT token"),
        ):

            initial_request = InitialQuestionsRequest(
                personal_info=sample_personal_info, jwt_token="invalid-token"
            )
