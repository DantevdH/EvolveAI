...
class TestDatabaseTrainingTypeHandling:
    ...
    @pytest.mark.asyncio
    async def test_save_training_plan_includes_training_type(
        self, sample_training_plan_data, mock_supabase_client
    ):
        ...
        db_service._get_authenticated_client = MagicMock(
    ...
    @pytest.mark.asyncio
    async def test_save_training_plan_fallback_training_type(
        ...
        monday_call = next(
            call for call in daily_training_calls if call["day_of_week"] == "Monday"
        )
        tuesday_call = next(
            call for call in daily_training_calls if call["day_of_week"] == "Tuesday"
        )
    ...
    @pytest.mark.asyncio
    async def test_get_training_plan_preserves_training_type(
        self, mock_supabase_client
    ):
        ...
        mock_weekly_schedules = [{"id": 1, "training_plan_id": 1, "week_number": 1}]
        ...
        monday_training = next(
            dt for dt in daily_trainings if dt["day_of_week"] == "Monday"
        )
        tuesday_training = next(
            dt for dt in daily_trainings if dt["day_of_week"] == "Tuesday"
        )
