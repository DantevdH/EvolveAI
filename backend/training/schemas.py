...
class ExerciseSchema(BaseModel):
    ...
    reps: str = Field(..., description="Rep range or duration, e.g., '8-12' or '45 seconds'")
...
class DailyWorkoutSchema(BaseModel):
    ...
    exercises: List[ExerciseSchema] = Field(
        ...
        description="List of exercises (empty if rest day)"
    )
...
class WorkoutPlanSchema(BaseModel):
    ...
    weekly_schedules: List[WeeklyScheduleSchema] = Field(
        ...
        min_items=1, 
        max_items=12, 
        description="1-12 weeks of workout schedules"
    )