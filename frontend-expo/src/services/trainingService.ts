...
export class TrainingService {
  ...
  static async generateWorkoutPlan(
    ...
    try {
      ...
      if (result.status === 'success') {
        console.log('✅ Workout plan generated by backend');
        
        // Instead of parsing the API response, fetch the complete workout plan from Supabase
        // This ensures we get all the proper IDs, relationships, and formatted data
        const workoutPlanResult = await this.getWorkoutPlan(userProfileId);
        
        if (workoutPlanResult.success && workoutPlanResult.data) {
          console.log('✅ Workout plan fetched from database');
          return {
            success: true,
            data: workoutPlanResult.data,
          };
        } else {
          console.error('❌ Failed to fetch generated workout plan from database');
          return {
            success: false,
            error: 'Workout plan was generated but could not be retrieved',
          };
        }
      } else {
        console.error('❌ Generation failed - invalid response');
        return {
          success: false,
  ...
  static async getWorkoutPlan(userProfileId: number): Promise<TrainingServiceResponse<TrainingWorkoutPlan>> {
    console.log('🔍 TrainingService: Starting workout plan fetch...', {
      userProfileId,
    ...
    try {
      ...
      if (error) {
        if (error.code === 'PGRST116') {
          console.log('ℹ️ TrainingService: No workout plan found (PGRST116) - this is expected for new users');
          return {
            success: false,
