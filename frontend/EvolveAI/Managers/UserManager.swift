...
class UserManager: ObservableObject {
   ...
   @Published var authToken: String? {
    ...
    /// Checks for a token and then fetches the user's core profile.
    func checkAuthenticationState() {
        if authToken == nil {
            self.isLoading = false
        } else {
            // The only thing UserManager checks is the UserProfile.
            fetchUserProfile()
        }
    }
    
    /// On login, fetch the UserProfile.
        ...
        networkService.login(credentials: credentials) { [weak self] result in
            DispatchQueue.main.async {
                ...
                case .success(let token):
                    ...
                    self?.fetchUserProfile() // Fetch the profile, not the plan
    ...
    /// Fetches ONLY the user's profile from the server.
    func fetchUserProfile() {

        #if DEBUG
        // In DEBUG mode, we can use a mock profile.
        ...
        isLoading = true
        // You'll need a networkService call that gets the UserProfile
        networkService.getUserProfile(authToken: token) { [weak self] result in
            DispatchQueue.main.async {
                ...
                case .failure:
                    self?.userProfile = nil // No profile exists
                }
                self?.isLoading = false
            }
        }
...
//     /// Checks if onboarding/workoutplan is completed on the server to determine onboarding status.
